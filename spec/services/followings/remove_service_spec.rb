  ...
  describe '#call' do
    ...
    context 'when no following relationship exists' do
      it 'returns an error message' do
        ...
        expect(result.errors).to include("Following relationship not found")
      end
    ...
    context 'when invalid parameters are provided' do
      it 'returns an error when neither users nor ID is provided' do
        service = described_class.new
        result = service.call

        expect(result.success?).to be false
        expect(result.errors).to include("Must provide either following ID or both follower and followed users")
      end

      it 'returns an error when only follower is provided' do
        service = described_class.new(follower: follower)
        result = service.call

        expect(result.success?).to be false
        expect(result.errors).to include("Must provide both follower and followed users")
      end

      it 'returns an error when only followed is provided' do
    ...
    context 'when deletion fails for some reason' do
      it 'returns an error message' do
        ...
        expect(result.success?).to be false
        expect(result.errors).to include("Failed to unfollow user")
      end
    end
  end
end
